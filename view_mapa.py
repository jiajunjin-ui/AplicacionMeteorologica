...
class ViewMapa(ViewBase):
    ...
    def setup_ui(self):
        ...
        btn_buscar = tk.Button(
            ...
            command=lambda: self.opera("1")
        )
        btn_buscar.grid(row=2, column=0, pady=5, sticky="ew")

        btn_buscar_pais = tk.Button(
            self.columna_izq,
            text="Mostrar pais",
            command=lambda: self.opera("2")
        )
        ...
        chk_temp = tk.Checkbutton(
            self.columna_izq,
            text="Temperatura",
            variable=self.mostrar_temperatura,
            command=self.actualizar_marcador_actual
        )
        chk_temp.grid(row=4, column=0, pady=5, sticky="w")

        chk_humedad = tk.Checkbutton(
            self.columna_izq,
            text="Humedad",
            variable=self.mostrar_humedad,
            command=self.actualizar_marcador_actual
        )
        chk_humedad.grid(row=5, column=0, pady=5, sticky="w")

        chk_viento = tk.Checkbutton(
            self.columna_izq,
            text="Viento",
            variable=self.mostrar_viento,
            command=self.actualizar_marcador_actual
        )
    ...
    def seleccionar_ciudad(self, nombre_ciudad):
        """Funcion que toma la referencia de la ciudad elegida por el usuario."""
